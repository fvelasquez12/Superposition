...
## Video
[Watch the video](https://youtu.be/drW9KE36o48)
...
We also implemented a deep reinforcement learning algorithm to benchmark against our quantum model, since the parametrized circuits are sometimes the quantum counterparts to neural networks. We first implemented the algorithm using convolutional neural network (CNN) which takes the whole screen as input, taken from the tutorials in [4]. We also implemented a simple neural network with one hidden layer to compare with the CNN and quantum model.
